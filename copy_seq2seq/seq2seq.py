...
from seq2seq import sequence_copy_loss
...
def embedding_tied_rnn_seq2seq(encoder_inputs,
  ...
  with variable_scope.variable_scope(
    ...
    if nest.is_sequence(zero_state):
      state = nest.pack_sequence_as(
          structure=zero_state, flat_seque implementation is based on http://arxiv.org/abs/1412.7449 (see below for
...
def embedding_attention_decoder(decoder_inputs,
  ...
  Args:
    ...
    output_projection: None or a pair (W, B) of output projection weights and
      ...
      output will first be multiplied by W and added B.
    feed_previous: Boolean; if True, only the first of decoder_inputs will be
      u   symbol) will be updated by back propagation. Embeddings for the symbols
      generated from the decoder itself remain unchanged. This parameter has
      no effect if feed_previous=False.
...
def one2many_rnn_seq2seq(encoder_inputs,
  ...
  decoders. Reference to multi-task sequenct).
      ...
      with variable_scope.variable_scope("one2many_decoder_" + str(
        ...
        if isinstance(feed_previous, bool):
          ...
          def filled_embedding_rnn_decoder(feed_previous):
            ...
            state_list = [state]
            if nest.is_se         outputs_len = len(decoder_inputs)
...
def sequence_loss(logits,
  ...
  Raises:
    ValueError: If len(logits) is different from len(targets) or len(weights).
  """
  with ops.na  batch_size = array_ops.shape(targets[0])[0]
      return cost / math_ops.cast(batch_size, cost.dtype)
    else:
      return cost
